...
class PriceMachine:
    ...
    def export_to_console(self):
        for index, product in enumerate(self.data, 1):
            print(
                f"{index}. Название: {product.get('название')}, Цена: {product.get('цена')}, Вес: {product.get('вес')},"
                f"Файл: {product.get('файл')}")
    ...
    def export_to_html(self, output_file_path=r'C:\Users\azatt\PycharmProjects\Practic\output.html'):
        if self.data:
            ...
            with open(output_file_path, 'w', encoding='utf-8') as file:
                ...
                for idx, row in enumerate(sorted_data, start=1):
                    ...
                    file.write(
                        f"<tr><td>{idx}</td><td>{item_name}</td><td>{row.get('цена', '')}</td>"
                        f"<td>{row.get('вес', '')}</td><td>{row.get('файл', '')}</td><td>{price_per_kg:.1f}</td></tr>"
                    )
                file.write('''
                       </table>
                   </body>
    ...
    def find_text(self, text):
        ...
        if sorted_results:
            ...
            for idx, result in enumerate(sorted_results, 1):
                print(
                    f"{idx}. Название: {result.get('название')}, Цена: {result.get('цена')}, Вес: {result.get('вес')},"
...
pm = PriceMachine()
pm.load_prices(r'C:\Users\azatt\PycharmProjects\Practic')
